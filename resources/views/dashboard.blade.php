...
<body>
    ...
    <div class="content">
        @if (auth()->guard('web')->check())
            <div class="row">
                <div class="col-md-12">
                    <h1 class="text-center">{{Auth::user()->roles->pluck('name')}}</h1>
                </div>
            </div>
                ...
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{$item->name}}</td>
                            <td>
                                <img src="{{ url('assets/img/cimages/'.$item->image) }}" alt="" width="40" height="40" class="rounded-circle">
                            </td>
                            <td>
    ...
    <script>
        $(document).on("click", "#editbutton", function(e) {
            ...
            var url = "{{ url('serviceedit') }}?ids=" + ids;
        ...
        $(document).on("click", "#deletebutton", function(e) {
            e.preventDefault();
            var ids = $(this).data('id');
            var url = "{{ url('servicedelete') }}?ids=" + ids;
            window.location.href = url;
        });
    </script>
</body>
